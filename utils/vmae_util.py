...
def init_distributed_mode(args):
    ...
    Log.info('| distributed init (rank {}): {}, gpu {}'.format(
...
def load_state_dict(model, state_dict, prefix='', ignore_missing="relative_position_index"):
    ...
    if len(ignore_missing_keys) > 0:
        Log.info("Ignored weights of {} not initialized from pretrained model: {}".format(
...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
    ...
    Log.info("Set warmup steps = %d" % warmup_iters)
    if warmup_epochs > 0:
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if loss_scaler is not None:
        ...
        if args.auto_resume and len(args.resume) == 0:
            ...
            Log.info("Auto resume checkpoint: %s" % args.resume)
        ...
        if args.auto_resume:
            ...
            if latest_ckpt >= 0:
                ...
                Log.info("Auto resume checkpoint: %d" % latest_ckpt)
