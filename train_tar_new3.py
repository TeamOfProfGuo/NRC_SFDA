...
from dataset.visda_data import data_load, moco_transform
...
def finetune_one_epoch(netF, netB, netC, dset_loaders, optimizer, epoch):
    ...
    for iter_num, batch_data in enumerate(dset_loaders["target"]):
        ...
        loss0 = compute_loss(plabel, prob_tar0, type=args.loss_type, weight=weight, soft_flag=args.plabel_soft)
        loss1 = compute_loss(plabel, prob_tar1, type=args.loss_type, weight=weight, soft_flag=args.plabel_soft)

        ce_loss = 2.0 * args.ce_wt * loss0 + 2.0 * (1-args.ce_wt) * loss1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ce_wt", type=float, default=0.5, help='weight of ce0 wrt. ce1')
