...
def data_load(args, moco_load=False):
    ...
    dset_loaders["target"] = DataLoader(dsets["target"], batch_size=train_bs, shuffle=True, num_workers=args.num_workers, drop_last=False)

    dsets["test"] = ImageList(txt_test, transform=image_test(), root=os.path.dirname(args.test_data_path), ret_idx=True)
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=train_bs * 3, shuffle=False, num_workers=args.num_workers, drop_last=False)

    if moco_load:
        dsets['target_moco'] = ImageList(txt_tar, transform=moco_transform, root=os.path.dirname(args.target_data_path), ret_idx=True)
        dset_loaders['target_moco'] = DataLoader(dsets['target_moco'], batch_size=train_bs, shuffle=True, num_workers=args.num_workers, drop_last=False)
