...
from dataset.oh_data import office_load, moco_transform, image_target, mn_transform, mw_transform, get_RandAug, \
    get_AutoAug, mr_transform
from model.model_util import bn_adapt, label_propagation, local_cluster, extract_feature_labels, extract_features, \
    get_affinity
...
def reset_data_load(dset_loaders, pred_prob, args, ss_load=None):
    ...
    data_target = ImageList(tar_list, transform=data_trans, root='../dataset/', ret_idx=True, pprob=pred_prob,
                            ret_plabel=True, args=args)
    dloader = DataLoader(data_target, batch_size=args.batch_size, shuffle=True, num_workers=args.worker,
                         drop_last=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--loss_type', type=str, default='dot', help='Loss function for target domain adaptation',
                        choices=['ce', 'sce', 'dot', 'dot_d'])
    parser.add_argument('--loss_wt', type=str, default='en5', help='CE/SCE loss weight: e|f|p|n, c|n, 0-9')
    parser.add_argument('--plabel_soft', action='store_false', default=True,
                        help='Whether to use soft/hard pseudo label')  #
    ...
    parser.add_argument('--lp_type', type=float, default=1.0,
