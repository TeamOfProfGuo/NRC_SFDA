...
from dataset.domain_data import domain_load, moco_transform, image_target, mn_transform, get_AutoAug, mw_transform, get_RandAug
...
def reset_data_load(dset_loaders, pred_prob, args,):
    ...
    t = map_name(tt)
    tar_list = "./dataset/data_list/domainnet-126/{}_list.txt".format(t)
    ...
    data_target = ImageList(tar_list, transform=data_trans, root='../dataset/domainNet', ret_idx=True, pprob=pred_prob, ret_plabel=True, args=args)
...
def train_target(args):
    dset_loaders = domain_load(args, ret_idx=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--class_num", type=int, default=126)
