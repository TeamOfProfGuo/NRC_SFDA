...
from model.model_util import bn_adapt, bn_adapt1, label_propagation, extract_feature_labels, get_affinity, stratified_split, compute_acc
...
def reset_data_load(dset_loaders, pred_prob, args, moco_load=False):
    ...
    dsets = ImageList(txt_tar, transform=data_trans, root=os.path.dirname(args.t_dset_path), ret_idx=True,
...
def analysis_target(args):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        # ============ feature extraction ============
        model.eval()
        pred_labels, feats, labels, pred_probs_raw = extract_feature_labels(dset_loaders["test"], model.netF,
                                                                            model.netB, model.netC, args, log, epoch)

        if epoch == 1 and (args.fuse_af >= 0 or args.feat_type == 'o'):
            n_splits = 5
            idx_all = stratified_split(labels, n_splits=n_splits)   # dict
            feats_splits, labels_splits, pred_probs_raw_splits, W_ori_splits = [], [], [], []
            for n_split in range(n_splits):
                idx = idx_all[n_split]
                feats_splits[n_split] = feats[idx]
                labels_splits[n_split] = labels[idx]
                pred_probs_raw_splits[n_split] = pred_probs_raw[idx]
                W_ori_splits[n_split] = get_affinity(feats_splits[n_split], args)

        # ============ label propagation ============
        if epoch > args.fuse_af + 1:
            fname = osp.join(args.output_dir, 'w{}.pickle'.format(epoch - args.fuse_af))
            with open(fname, 'rb') as f:
                W0_splits = pickle.load(f)
        ...
        for n_split in range(n_splits):
            if epoch <= 1:
                ...
                W0 = W_ori_splits[n_split]
            else:
                W0 = W0_splits[n_split]

                pred_labels, pred_probs, mean_acc, acc, W_new = label_propagation(
                    pred_probs_raw_splits[n_split],
                    feats_splits[n_split],
                    labels[n_split],
                    args, log,
                    W0=W0,
                    alpha=0.99, max_iter=20, ret_acc=True, ret_W=True)
            W_new_splits[n_split] = W_new
            pred_probs_splits[n_split] = pred_probs

        pred_probs_splits = np.concatenate(pred_probs_splits, axis=0)
        indices_splits = np.concatenate([idx_all[n_split] for n_split in range(n_splits)])
        pred_probs = pred_probs_splits[indices_splits]

        if args.fuse_af > 0:
            fname = osp.join(args.output_dir, 'w{}.pickle'.format(epoch))
            with open(fname, 'wb') as f:
                pickle.dump(W_new_splits, f)

        # ===== compute acc
        pred_labels= np.argmax(pred_probs, 1)
        acc = float(np.sum(pred_labels == labels)) / len(labels)
        mean_acc, _ = compute_acc(labels, pred_labels)
        log('--->After label propagation Acc: {:.2f}%, Mean Acc: {:.2f}%'.format(acc * 100, mean_acc * 100))
...
def finetune_one_epoch(model, dset_loaders, optimizer, epoch=None):
    ...
    if args.dset == 'visda-2017':
        mean_acc, classwise_acc, acc, cm = cal_acc(dset_loaders['test'], model.netF, model.netB, model.netC, flag=True,
                                                   ret_cm=True)
        log('After fine-tuning, Acc: {:.2f}%, Mean Acc: {:.2f}%,'.format(acc * 100,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--T_decay', type=float, default=0.0,
    ...
    parser.add_argument('--loss_type', type=str, default='dot', help='Loss function',
                        choices=['ce', 'sce', 'dot', 'dot_d'])
    parser.add_argument('--loss_wt', type=str, default='en5',
                        help='CE/SCE loss weight: e|p|n, c|n (classwise weight), 0-9')
    ...
    for i in range(len(names)):
        ...
        args.output_dir = osp.join(args.output, args.dset, args.exp_name,
