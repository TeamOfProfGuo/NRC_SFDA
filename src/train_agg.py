...
class Trainer(object):
    ...
    def finetune_one_epoch(self, epoch, get_acc=True):
        ...
        for iter_num, batch_data in enumerate(self.dataloaders["target_moco"]):
            ...
            if args.loss_wt:
                ce_loss = compute_loss(plabel, prob_tar, type=self.args.loss_type, weight=weight, cls_weight=True)
            else:
    ...
    def train(self):
        ...
        for epoch in range(self.args.max_epochs):
            ...
            # get final acc
            acc_tar = self.finetune_one_epoch(epoch, get_acc=True)  # add 14 because we have 14 epoch of init train of aggregator
